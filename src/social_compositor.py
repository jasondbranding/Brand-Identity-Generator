...
SOCIAL_SPECS = {
    "collab_post": {
        ...
        "constraints": [
            ...
            "if any pattern or texture is used as background panel: apply a semi-transparent dark or light overlay at 40–60% opacity over it so the logo and × symbol remain clearly legible",
    ...
    "announcement_post": {
        ...
        "constraints": [
            ...
            "if a pattern or texture is used as background: apply a dark or light semi-transparent color overlay at 40–60% opacity on top of it — the overlay color should complement the brand palette",
            "strong contrast: all text must be clearly readable against background — minimum contrast ratio 4.5:1",
    ...
    "ads_post": {
        ...
        "constraints": [
            "strict 16:9 ratio, 1920×1080px",
            "left 55%: filled with the provided brand pattern or background asset — apply a dark or light semi-transparent overlay at 40–60% opacity on top of the pattern to subdue it slightly — do not add any text to this side",
            "right 45%: solid color panel (brand primary or complementary dark tone) — this panel needs NO overlay, it is already solid",
            ...
            "overlay on left panel: choose dark overlay if pattern is light, light overlay if pattern is dark — goal is visual harmony between left and right panels",
            "clean hard vertical edge between visual and copy panel",
            "no additional text or decorative elements on either side",
    ...
    prompt_dict = {
        "IMAGE_GEN_V1": {
            ...
            "global_design_principles": [
                "Legibility first: text and logo must always be clearly readable",
                "Pattern/texture as background: always apply a semi-transparent overlay (dark or light, 40–60% opacity) over any pattern or textured background before placing text or logos on top",
                "Overlay color: choose dark overlay (#000 at 40–60% opacity) for light patterns, light overlay (#fff at 40–60% opacity) for dark patterns — match brand palette where possible",
                "No raw pattern behind text: naked pattern texture behind text is not acceptable",
            ],
            "brand": {
                "name": brand_name,
                "direction": direction_name,
