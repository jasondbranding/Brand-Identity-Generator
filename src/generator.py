...
def _create_logo_variants(logo_path: Path, asset_dir: Path) -> dict:
    ...
    Steps:
      ...
      4. Convert black and white PNGs to SVG format using vtracer       → logo_white.svg, logo_black.svg

    Returns {"logo_transparent": Path, "logo_white": Path, "logo_black": Path, "logo_white_svg": Path, "logo_black_svg": Path}
    or {} on any failure.
    """
    try:
        ...
        try:
            ...
            # Replace exactly the hex colors typically generated by vtracer for the main shapes
            ...
            console.print(f"  [yellow]⚠ SVG vectorization failed: {ve}[/yellow]")

        console.print("  [dim]  logo variants → transparent / white / black (PNG & SVG)[/dim]")
        ...
        if svg_white_path.exists(): result["logo_white_svg"] = svg_white_path
        if svg_black_path.exists(): result["logo_black_svg"] = svg_black_path
